...
eval $( $CROW_TO_SH expand:diag_table_header ) > input.nml
eval $( $CROW_TO_SH DIAG_TABLE=DIAG_TABLE )
cat $DIAG_TABLE >> diag_table

# NEMS and FV3 namelists:
eval $( $CROW_TO_SH expand:input_nml ) > input.nml
eval $( $CROW_TO_SH expand:nems_configure ) > nems.configure
eval $( $CROW_TO_SH expand:model_configure ) > model_configure
